...
GLM 配额是用户的硬约束。推荐任何 flow 之前，先看项目是否声明了「云端流水线」约定（查 `AGENTS.md` / `NEXT.md` / ADR，例如 AI 分身浏览器项目的 `docs/adr/0005`）：

- **代码写作类工作**（`/implement` 的写码环节、`/prototype`、一次性脚本）→ 项目有此约定时，不要在本机跑写码 flow，改为产出「给云端沙箱（Z.ai 网页版 GLM）的任务书」让用户粘贴执行；产物经 Git 回流后，本机只做构建、验证、收尾。
- **验证 / 审查 / 流程类**（`/code-review`、`/triage`、`/wayfinder`、`/diagnosing-bugs`、模拟器验证）→ 一律本机执行：云端沙箱没有这些 skills、也没有构建环境。
- 项目没有此类约定时，按下面正常路由走。
