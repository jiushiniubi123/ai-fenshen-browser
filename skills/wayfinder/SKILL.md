---
name: wayfinder
description: "把单个 agent session 装不下的一大块工作规划成 issue tracker 上的 decision tickets shared map，并逐一解决，直到通往 destination 的路清晰。适用于用户面对 greenfield 项目或单个 session 装不下的大型模糊任务、不确定从何下手，或要求先铺决策地图再动工时。"
---
